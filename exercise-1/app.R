...
my.ui <- fluidPage(
  ...
  numericInput('num.input1', label = "Price(in dollars)", min = 0, value =0),

  # Include a `numericInput()` widget with the label "Quantity"
  # It should have a default value of 1 and a minimum value of 1
  numericInput('num.input2',label = "Quantity", value = 1, min = 1), 
...
my.server <- function(input, output) {
  ...
  output$my.cost <- renderText({
    
    return(input$num.input1 * input$num.input2)
  })
